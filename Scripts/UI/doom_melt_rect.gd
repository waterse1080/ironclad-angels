extends ColorRect
class_name DoomMeltRect

var melting := false
var timer := 0.0 # Incremented by melt_speed then applied to the timer variable in the shader

@export var x_resolution := 200.0 # Value must be greater or equal to y_offsets.length() in the shader
@export var max_offset := 2.5
@export var melt_speed := 1
